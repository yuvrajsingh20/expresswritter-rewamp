...
function AdminUsers() {
  ...
  const USERS = [
    { id: 'USR-001', name: 'Admin User', email: 'admin@xpresswriters.com', role: 'Super Admin', status: 'Active', lastLogin: '2 min ago', mfa: true },
    { id: 'USR-002', name: 'Sarah Mitchell', email: 'sarah@xpresswriters.com', role: 'Support Manager', status: 'Active', lastLogin: '1h ago', mfa: true },
    { id: 'USR-003', name: 'Ravi Sharma', email: 'ravi@xpresswriters.com', role: 'Finance', status: 'Active', lastLogin: '3h ago', mfa: false },
    { id: 'USR-004', name: 'Nadia Kowalski', email: 'nadia@xpresswriters.com', role: 'Content QC', status: 'Active', lastLogin: '1d ago', mfa: true },
    { id: 'USR-005', name: 'Ben Torres', email: 'ben@xpresswriters.com', role: 'Support Agent', status: 'Inactive', lastLogin: '7d ago', mfa: false },
  ];

  const ROLES = [
    { role: 'Super Admin', color: '#ef4444', perms: ['All access', 'Billing', 'User management', 'System settings'] },
    { role: 'Support Manager', color: '#f59e0b', perms: ['Tickets', 'Writer management', 'Order management', 'Reports'] },
    { role: 'Finance', color: '#22c55e', perms: ['Payments', 'Revenue', 'Payouts', 'Currency'] },
    { role: 'Content QC', color: '#3b82f6', perms: ['Orders', 'Writer profiles', 'Quality review'] },
    { role: 'Support Agent', color: '#8b5cf6', perms: ['Tickets', 'Client messages', 'Basic order view'] },
  ];

  const [newUser, setNewUser] = React.useState({ name: '', email: '', role: 'Support Agent', sendInvite: true });

  return (
    <div>
      <SectionHeader
        title="User & Access Management"
        subtitle="Manage admin accounts, roles, permissions and login settings."
      ...
      {showCreate && (
        <Card style={{ marginBottom: 20, padding: 16, border: '1px solid var(--border-teal)', animation: 'fadeUp .25s ease' }}>
          <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 14, color: 'var(--teal-light)' }}>Create New Admin Account</div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14 }}>
            <Input label="Full Name" value={newUser.name} onChange={v => setNewUser(u => ({ ...u, name: v }))} placeholder="Jane Smith" />
            <Input label="Email Address" value={newUser.email} onChange={v => setNewUser(u => ({ ...u, email: v }))} placeholder="jane@xpresswriters.com" type="email" />
            <Select label="Role" value={newUser.role} onChange={v => setNewUser(u => ({ ...u, role: v }))} options={ROLES.map(r => r.role)} />
          </div>
          <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
            <Btn onClick={() => { setShowCreate(false); setSaved(true); setTimeout(() => setSaved(false), 2500); }}>Send Invite</Btn>
      ...
      {tab === 'Users' && (
        <Card style={{ animation: 'fadeIn .3s ease' }}>
          <Table
            cols={['User', 'Email', 'Role', 'Status', 'Last Login', 'MFA', 'Actions']}
            rows={USERS.map(u => [
              ...
              <Pill label={u.status} color={u.status === 'Active' ? 'var(--green)' : 'var(--text-dim)'} />,
              <span style={{ fontSize: 11, color: 'var(--text-dim)' }}>{u.lastLogin}</span>,
              <span style={{ fontSize: 13 }}>{u.mfa ? '🔒' : '⚠️'}</span>,
              <div style={{ display: 'flex', gap: 6 }}>
                <Btn small variant="ghost">Edit</Btn>
                <Btn small variant={u.status === 'Active' ? 'danger' : 'outline'}>{u.status === 'Active' ? 'Disable' : 'Enable'}</Btn>
              </div>,
            ])}
          />
        </Card>
...
Object.assign(window, { AdminTheme, AdminWorkflow, AdminUsers });

  ...
  return (
    <div style={{ padding: isMobile ? '16px 20px' : '32px 36px', overflowY: 'auto', height: '100%', animation: 'fadeIn 0.3s ease' }}>
      <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', alignItems: isMobile ? 'flex-start' : 'center', justifyContent: 'space-between', gap: isMobile ? 16 : 0, marginBottom: 28 }}>
        ...
        <button 
          onClick={markAllRead}
          style={{ background: 'none', border: '1px solid var(--border2)', borderRadius: 8, padding: '8px 16px', fontSize: 13, color: 'var(--text-muted)', cursor: 'pointer' }}
        >
          Mark all as read
        </button>
      </div>
          ...
          >
            ...
            <div style={{ flex: 1 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 4 }}>
                <h3 style={{ fontSize: 15, fontWeight: 600, color: 'var(--text)' }}>{n.title}</h3>
                <span style={{ fontSize: 12, color: 'var(--text-dim)' }}>{n.time}</span>
              </div>
              <p style={{ fontSize: 13, color: 'var(--text-muted)', lineHeight: 1.5, marginBottom: 8 }}>{n.msg}</p>
              {n.type === 'order' && (
                <button style={{ background: 'none', border: 'none', color: 'var(--teal-light)', fontSize: 12, fontWeight: 600, cursor: 'pointer', padding: 0 }}>View Order Details →</button>
              )}
            </div>
          </div>
        )) : (

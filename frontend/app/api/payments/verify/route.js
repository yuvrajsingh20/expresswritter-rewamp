...
export async function POST(req) {
  try {
    ...
    if (cashfree_order_id) {
      // 1. Contact Cashfree ledger directly to verify status
      const cfOrder = await fetchCashfreeOrder(cashfree_order_id);
      const isPaid = cfOrder.order_status === "PAID";

      if (!isPaid) {
        return NextResponse.json({ message: "Payment not completed on Cashfree" }, { status: 400 });
      }

      // Check if this was a custom checkout invoice Payment first
      ...
      if (directPayment) {
        ...
        await prisma.$transaction(async (tx) => {
          await tx.payment.update({
            ...
            data: {
              status: "captured",
              cashfreePaymentId: cfOrder.cf_order_id?.toString() || cashfree_order_id,
            }
          });
          ...
          if (directPayment.checkoutSessionId) {
            ...
            const createdProject = await tx.project.create({
              data: {
                ...
                status: "CREATED",
                serviceType: session.services?.[0]?.name || "Custom Package",
              }
            });
            ...
            await tx.projectLog.create({
              data: {
                action: "Payment Completed for Custom Package (Cashfree) - Awaiting Admin Assignment",
                projectId: createdProject.id,
      ...
      // Otherwise, standard session checkout
      const session = await getAndDeletePaymentSession(cashfree_order_id);

      if (!session) {
        const existingOrder = await prisma.order.findFirst({
          where: { cashfreeId: cashfree_order_id },
        });
      ...
      const project = await prisma.$transaction(async (tx) => {
        ...
        await tx.projectLog.create({
          data: {
            action: "Payment Completed (Cashfree) - Awaiting Admin Assignment",
            projectId: createdProject.id,
            userId: studentId,
          },
        });

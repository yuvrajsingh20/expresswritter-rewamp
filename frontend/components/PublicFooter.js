  ...
  return (
    <>
      ...
      <div style={{ padding: '40px 32px', background: 'linear-gradient(135deg,rgba(13,148,136,0.06),transparent)', borderTop: '1px solid var(--border)', borderBottom: '1px solid var(--border)' }}>
        <div style={{ maxWidth: 1320, margin: '0 auto', display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '24px 60px', textAlign: 'center' }}>
          {[
            ...
            <div key={t} style={{ flex: '1 1 200px', maxWidth: 260 }}><div style={{ fontSize: 28, marginBottom: 7 }}>{i}</div><div style={{ fontSize: 13, fontWeight: 700, marginBottom: 3 }}>{t}</div><div style={{ fontSize: 11.5, color: 'var(--text-muted)', fontWeight: 300 }}>{d}</div></div>
          ))}
        </div>
      </div>
      ...
      <footer style={{ borderTop: '1px solid var(--border)', background: 'var(--bg)', paddingTop: 60, paddingBottom: 30 }}>
        <div style={{ maxWidth: 1320, margin: '0 auto', padding: '0 32px', display: 'flex', flexWrap: 'wrap', gap: 60, justifyContent: 'space-between', marginBottom: 60 }}>
          <div style={{ maxWidth: 300 }}>
            ...
            <div style={{ display: 'flex', gap: 14, alignItems: 'center', marginTop: 8 }}>
              <a href="https://linkedin.com" target="_blank" rel="noopener noreferrer" style={{ color: 'var(--text-dim)', fontSize: 13, textDecoration: 'none', fontWeight: 500, transition: 'color 0.15s' }} onMouseEnter={(e) => e.target.style.color = 'var(--teal)'} onMouseLeave={(e) => e.target.style.color = 'var(--text-dim)'}>
                LinkedIn
              </a>
              <span style={{ color: 'var(--border)', fontSize: 12 }}>·</span>
              <a href="https://instagram.com" target="_blank" rel="noopener noreferrer" style={{ color: 'var(--text-dim)', fontSize: 13, textDecoration: 'none', fontWeight: 500, transition: 'color 0.15s' }} onMouseEnter={(e) => e.target.style.color = 'var(--teal)'} onMouseLeave={(e) => e.target.style.color = 'var(--text-dim)'}>
                Instagram
              </a>
              <span style={{ color: 'var(--border)', fontSize: 12 }}>·</span>
              <a href="https://wa.me/918823830076" target="_blank" rel="noopener noreferrer" style={{ color: 'var(--text-dim)', fontSize: 13, textDecoration: 'none', fontWeight: 500, transition: 'color 0.15s' }} onMouseEnter={(e) => e.target.style.color = 'var(--teal)'} onMouseLeave={(e) => e.target.style.color = 'var(--text-dim)'}>
                WhatsApp
              </a>
            </div>
          </div>
        ...
        <div style={{ borderTop: '1px solid var(--border)', paddingTop: 24, padding: '24px 32px 0' }}>
          <div style={{ maxWidth: 1320, margin: '0 auto', display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', gap: 20, alignItems: 'center' }}>
            <div style={{ fontSize: 11.5, color: 'var(--text-dim)' }}>© 2024 Xpresswriters. All rights reserved. Made with care in India.</div>
          </div>
        </div>
      </footer>
    </>
